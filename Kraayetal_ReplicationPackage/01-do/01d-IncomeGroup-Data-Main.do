*******************************************************************************
/*
This do file produces WB income group level data for user defined year 
	used for the figures and charts in Kraay r al.
Definition of income group used is the income group defined in the data year.
...
ren year year_data
merge m:1 code year_data using "${rawdatapath}CLASS.dta", keep(1 3) nogen keepusing(incgroup_historical)
ren year_data year

keep if year == `year' 									// estimates for particular year
egen id = group(incgroup_historical)
...
collapse *25 *215 *365 *685 i gini sen mld atk* awi* ybar* (rawsum) pop [aw=pop], by(incgroup_historical year)
...
order incgroup_historical year pop w* c* p* i ybar* fgt* watt* gini sen mld atk* awi* 
...
compress
save "${intermediatedatapath}IncomeGroupData_Main`year'.dta", replace
